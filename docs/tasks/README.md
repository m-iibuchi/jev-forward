...
- **作業の指示はタスク番号で行う**。実装のような予定された作業もここに起票してから着手する（task 007・008・018〜021 がその例）。1タスク＝1セッション分の粒度にし、手順や貼る指示文は `docs/実装セッション指示.md` や `docs/detailed/` に置いてタスクからリンクする（本文を二重に持たない）
...
| 018 | [実装 M1 前半（セッション 1a：共通規約・凍結ログ）](018-実装M1前半.md) | open | claude | 019、以降の全実装 |
